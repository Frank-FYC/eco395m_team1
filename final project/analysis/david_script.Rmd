...
#Effect of HS on PPVT Score
``` {r}
# Time-fixed effects modeling code

final <- read.csv("~/Documents/GitHub/eco395m_team_awesome/final project/data/finaldataset.csv", header=TRUE)
na.omit(final)
View(final)


lm1 <- lm(LogInc_0to3~somecollege, data=final)

colnames(final)

glm1 = glm(HSGrad~headstart+BirthWeight+hsgrad+FirstBorn  , data=final, family=binomial)
glm2 = glm(HSGrad~ (.) - logBW - compscoreage6 - compscoreage7 - compscoreage8 - compscoreage9- compscoreage10- compscoreage11- compscoreage12- compscoreage13- compscoreage14- compscoreage15- compscoreage16- compscoreage17- compscoreage18, data=final, family=binomial)
coef(lm1)
coef(glm1)
library(stargazer)
...
Effect of HS on HSGrad
Effect of HS on college enrollment
```
